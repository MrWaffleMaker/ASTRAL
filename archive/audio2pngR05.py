...
def plot_polar_spectrogram(hsv_array, output_filename="polar_spectrogram.png",dpi=800):
    num_rows, num_cols = hsv_array.shape[:2]
    
    max_radius = calc_req_radius(band_to_numbins(6),dpi)
    ...
    plt.savefig(output_filename, transparent=True, bbox_inches="tight", pad_inches=0, dpi=dpi)
    plt.close(fig)

def audio2png(input_file,output_file='polar_spectrogram.png',start_time=None, end_time=None):
    ...
    plot_polar_spectrogram(hsv_array,output_file)
    

if __name__ == "__main__":
    audio2png('pizza.mp3','pizzaspec.png',0,30)
